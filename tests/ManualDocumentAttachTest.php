...
/**
 * Два рубежі привʼязування накладної, виписаної вручну: звести набраний номер
 * до канонічного вигляду і не пустити номер, якого в Новій Пошті немає.
 *
 * Друге неочевидне: на вигаданий номер API відповідає `success: true` і віддає
 * документ зі `StatusCode` 3, тож перевірка за `success` пропустила б помилку
 * друку, а неіснуюча накладна дійшла б і до експорту, і до покупця.
 */
class ManualDocumentAttachTest extends TestCase
{
    ...
    /**
     * `StatusCode` 3 — «такого номера немає», а не помилка запиту.
     *
