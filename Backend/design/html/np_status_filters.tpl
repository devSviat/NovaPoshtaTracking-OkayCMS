{if $all_np_status}
    <div class="col-md-3 col-lg-3 col-sm-12">
        <select name="np_status" class="selectpicker form-control" onchange="location = this.value;">
            <option value="{url controller=OrdersAdmin np_status=null status=$status_id keyword=$keyword id=null page=null label=$label_id from_date=$from_date to_date=$to_date}" {if !$np_status_id}selected{/if}>{$btr->sviat__novaposhta_tracking__all_delivery_statuses|escape}</option>
